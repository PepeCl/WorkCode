...
while (True):
    ...
    if (seleccion == 7):
        lista_autos_usuario = []
        for autos in lista_autos:
            if "nombre usuario" in autos.keys():
                lista_autos_usuario.append(autos)
        if not lista_autos_usuario:
            print("no hay nada")
        else:
            for autos_usuario in lista_autos_usuario:
                print(f"Marca = {autos_usuario["marca"]}\nModelo = {autos_usuario["modelo"]}\nAño = {autos_usuario["año"]}\nPatente = {autos_usuario["patente"]}\nColor = {autos_usuario["color"]}\n**********")
